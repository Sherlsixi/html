...
async function render(params = {}) {
  ...
  document.querySelector(".page-now").innerHTML = `第${articlesList.page}页`;

  conditions.page = articlesList.page;
  conditions.channel_id = params.channel_id;
  conditions.status = params.status;
  const lastPage = Math.ceil(articlesList.total_count / articlesList.per_page);
...
document.querySelector(".art-list").addEventListener("click", async (e) => {
  if (e.target.classList.contains("del")) {
    ...
    if (children.length === 1 && conditions.page !== 1) {
      conditions.page--;
      document.querySelector(".page-now").innerHTML = `第${conditions.page}页`;
    }
    render(conditions);
  }
});
